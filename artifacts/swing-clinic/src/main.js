...
const Q_KEYS = ['sliceFrequency', 'missDirection', 'balanceLoss', 'fatShots', 'sevenIronDistance', 'clubType'];
...
function renderResults(data) {
  ...
  drillsToShow.forEach((drill, i) => {
    ...
    card.querySelector('.drill-complete-btn').addEventListener('click', function() {
...
function renderProgress() {
  ...
  const streakSub = streak >= 7 ? 'You\'re on fire! Keep it going.' :
                    streak >= 3 ? 'Great consistency!' :
                    streak === 1 ? 'Start a streak — come back tomorrow!' :
                    'Analyse today to start a streak!';

  // Drills completed this week
  const weekAgo = Date.now() - 7 * 86400000;
  const recentAnalyses = analyses.filter(a => new Date(a.timestamp).getTime() >= weekAgo);
...
function shareScore(data) {
  ...
  if (navigator.share) {
    navigator.share({ title: 'Swing Clinic', text, url: 'https://swing-clinic.replit.app' }).catch(() => {});
  } else {
...
function isDrillDone(analysisTimestamp, drillIndex) {
  return localStorage.getItem(getDrillKey(analysisTimestamp, drillIndex)) === '1';
}
...
function getStreak() {
  ...
  const days = [...new Set(analyses.map(a => a.timestamp.slice(0, 10)))].sort().reverse();
  let streak = 0;
...
// ── Achievements ───────────────────────────────────
const ACHIEVEMENTS = [
  { id: 'first',     icon: '🏌️', title: 'First Swing',    desc: 'Complete your first analysis',     check: (a)        => a.length >= 1 },
  { id: 'five',      icon: '🔥', title: 'Five Sessions',  desc: '5 analyses completed',             check: (a)        => a.length >= 5 },
  { id: 'ten',       icon: '⭐', title: 'Dedicated',      desc: '10 analyses completed',            check: (a)        => a.length >= 10 },
  { id: 'score60',   icon: '📈', title: 'Breaking 60',    desc: 'Score 60 or higher',               check: (a)        => a.some(x => x.overallScore >= 60) },
  { id: 'score70',   icon: '🎯', title: 'Solid Game',     desc: 'Score 70 or higher',               check: (a)        => a.some(x => x.overallScore >= 70) },
  { id: 'score80',   icon: '🏆', title: 'Elite Swing',    desc: 'Score 80 or higher',               check: (a)        => a.some(x => x.overallScore >= 80) },
  { id: 'streak3',   icon: '🔥', title: 'On a Roll',      desc: '3-day practice streak',            check: (a, s)     => s >= 3 },
  { id: 'streak7',   icon: '💎', title: 'Week Warrior',   desc: '7-day practice streak',            check: (a, s)     => s >= 7 },
  { id: 'improved',  icon: '📊', title: 'Improving',      desc: 'Score higher than first session',  check: (a)        => a.length >= 2 && a[a.length - 1].overallScore > a[0].overallScore },
];
